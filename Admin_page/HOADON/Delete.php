...
if (!isset($_GET["id"])) {
    ...
    echo "<script>window.location.href = './Index.php?alert=" . urlencode($alert) . "&alert_type=" . urlencode($alert_type) . "'</script>";
}
...
if (isset($_POST['delete'])) {
    ...
    while ($checker = mysqli_fetch_row($allForeignID)) {
        if ($_GET["id"] == $checker[0]) {
            ...
            echo "<script>window.location.href = './Delete.php?alert=" . urlencode($alert) . "&alert_type=" . urlencode($alert_type) .  "&id=" . urlencode($id) ."'</script>";
        }
    }
